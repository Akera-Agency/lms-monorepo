  ...
  const renderHeader = () => (
    <div className={cn('relative z-20 flex gap-2', icon && 'flex-row')}>
      ...
      <div className="flex w-full flex-col gap-0.5">
        ...
        <DialogDescription
          className={cn('text-sm font-medium text-secondary-text', descriptionClassName)}
        >
          {description}
        </DialogDescription>
      </div>
    ...
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        ...
        <DialogContent className={cn('w-full px-0 sm:max-w-[45rem]', className)}>
          <DialogHeader className={cn('px-5 pb-3', headerClassName)}>{renderHeader()}</DialogHeader>
          {separator && <Separator />}
