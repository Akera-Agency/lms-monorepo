...
export const infinityPagination = <T>(
  ...
  return {
    ...
    meta: {
      page: options.page,
      limit: options.limit,
      total_pages: options.total_count > 0 ? Math.ceil(options.total_count / options.limit) : 0,
      has_next_page:
        options.limit === data.length && options.total_count > options.limit * options.page,
