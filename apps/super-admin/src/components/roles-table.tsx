...
// import { useTenant } from "@/hooks/use.super-tenant";
// import {
//   type ColumnDef,
...
//   const columns = useMemo<ColumnDef<Role>[]>(() => getColumns({ setRoles }), [setRoles]);
...
//   return (
//     <div className="flex flex-col h-full px-10 py-4 overflow-auto custom-hidden-scrollbar">
//       {loading ? (
//         <LoadingSkeleton />
//       ) : roles.length === 0 ? (
//         <EmptyState
...
//                       <TableHead
//                         key={header.id}
//                         className="text-neutral-300 lg:text-sm text-xs px-2.5 py-2"
//                       >
...
//               {hasSelected && (
//                 <ConfirmDeleteDialog
//                   trigger={<Trash2 className="md:h-6 md:w-6 h-5 w-5 text-red-600 cursor-pointer" />}
...
// function PaginationControls({ table }: { table: TanstackTable<Role> }) {
//   return (
...
//         onClick={() => table.previousPage()}
//         disabled={!table.getCanPreviousPage()}
//         className="cursor-pointer md:hidden flex w-2 h-2"
//       >
//         <ChevronLeft />
//       </Button>
//       <Button
...
//               }
//               title="Confirm Deletion"
//               description={`Are you sure you want to delete role "${row.original.name}"?`}
//               closeText="Cancel"
//               confirmText="Delete"
//               onConfirm={async () => {
